...
library(lubridate)
library(hereR)
# ggspatial handles basemap tiles via annotation_map_tile() — no API key needed
...
# -------------------------------------------------------------
# 2. Basemap — OSM tiles via ggspatial (no API key, no terra)
# -------------------------------------------------------------

# annotation_map_tile() from ggspatial fetches OSM tiles on demand.
# It is added as the first layer in every ggplot() call.
# zoom = 10 for province-wide maps; zoom = 13 for facility-level maps.
# Tiles are cached locally after first download.
...
# Facility-level basemap uses zoom = 13 via annotation_map_tile inside each plot
...
p_fac_iso <- ggplot() +
  annotation_map_tile(type = "osm", zoom = 13, progress = "none") +
...
viviendas <- st_read("data/GEODATABASE_NACIONAL_2021/GEODATABASE_NACIONAL_2021.gdb/",
                     layer = "viv_p")


viviendas <- viviendas %>% 
  filter(str_detect(loc, "^17")) %>% 
  mutate(tipo = case_when(
    cod_otros %in% c("CAMPO DEPORTIVO", "CASA COMUNAL" ,
                     "GASOLINERA", "PARQUE", "PLAZA",
                     "TEMLO RELIGIOSO", "TEMPLO RELIGIOSO"
                      ) ~ "Amenities",
     str_detect(cod_otros,"EDIFICIO") ~ "Building",
    cod_otros == "ESTABLECIMIENTO DE SALUD" ~ "Health facility",
    TRUE ~ "housing"
    )
  )
...
viviendas <- viviendas %>%
  ...
  st_transform(st_crs(prr)) 

# Ver extent de cada capa
st_bbox(viviendas)
st_bbox(prr)

# Ver qué devuelve annotation_map_tile — a veces falla silenciosamente
# Prueba un ggplot mínimo sin tile:
viviendas_plot <- ggplot() +
  annotation_map_tile(type = "osm", zoom = 10, progress = "none") +
  geom_sf(data = prr, color = "darkgrey", alpha = 0.5) +
  geom_sf(data = viviendas, aes(color = tipo), alpha = 0.2, size = 0.5) +
  geom_sf(data = iso_sf %>% filter(!is.na(quintil)),
          aes(fill = quintil),
          colour = "white", linewidth = 0.04, alpha = 0.5,
          inherit.aes = FALSE) +
  scale_fill_viridis_d(name = "Accessibility\nquintile", na.value = "grey80") +
  theme_minimal()

ggsave("output/maps/10_viviendas_iso.png",
       viviendas_plot, width = W, height = H, dpi = DPI, bg = "white")


viviendas_plot2 <- ggplot() +
  annotation_map_tile(type = "osm", zoom = 10, progress = "none") +
  geom_sf(data = prr, color = "darkgrey", alpha = 0.5) +
  geom_sf(data = viviendas, aes(color = tipo), alpha = 0.2, size = 0.5) +
  geom_sf(data = buf_sf %>% filter(!is.na(quintil)),
...
ggsave("output/maps/09_viviendas_buff_.png",
       viviendas_plot2, width = W, height = H, dpi = DPI, bg = "white")
# +
...
tbl_medicos <- parroquias %>%
  summarise(
    `Parishes`              = n(),
    `Total doctors`      = sum(medicos, na.rm = TRUE),
...
gt_tbl3 <- tbl_medicos %>%
  gt() %>%
  tab_header(title = "Doctor supply by specialty — Pichincha parishes") %>%
  fmt_number(columns = Value, decimals = 0) %>%
