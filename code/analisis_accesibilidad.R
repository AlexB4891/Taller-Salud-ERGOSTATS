...
bbox_pichincha <- st_bbox(df) %>%
  setNames(c("left", "bottom", "right", "top"))

# Register your Stadia Maps key if needed:
...
bbox_fac <- st_bbox(iso_ejemplo) %>%
  setNames(c("left","bottom","right","top"))
basemap_fac <- get_stadiamap(bbox_fac, zoom = 13, maptype = "stamen_toner_lite")
